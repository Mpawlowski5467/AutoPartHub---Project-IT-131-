...
router.get('/cart', function(req, res, next) {
    ...
    let query = "SELECT Product_ID, Supplier_ID, Productname , ItemSold , RetailPrice , Packaging , IsDiscontinued FROM ProductInfo WHERE ID IN (" + req.session.cart + ") order by find_in_set(Product_ID, '" + req.session.cart + "');";
    // execute query
    db.query(query, (err, result) => {
